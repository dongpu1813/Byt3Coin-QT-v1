...
#if defined(NDEBUG)
# error "Byt3coin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x687d1a6cf3e16dbdf02086d4057d700438d771f556a14224449dcbf0e8f259b5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Byt3coin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Byt3coin Signed Message:\n";
...
{
    // Byt3coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Byt3coin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Byt3coin: 1 days
static const int64 nTargetSpacing = 1.1 * 60; // Byt3coin: 1.1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Byt3coin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Byt3coin: Special short-term limits to avoid 10,000 BDB lock limit:
    if (GetBlockTime() < 1376568000)  // stop enforcing 13 August 2018 00:00:00
    {
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6e551505991e534521f2dc278899de9c6754aa64454580264c2d3122b9bbec03");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC World News 14/Aug/2018 Turkey raises interest rates to 24%";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042b1e381a61abc13a663ebde7f324b18a8abd90ce4ca2d2d17a9360053e35119f16a2b239830b03e46f3bfbe9e0b69a2367e13b53504da18e51b66126833b69be") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1536874109;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086831683;

        if (fTestNet)
        {
            block.nTime    = 1536874112;
            block.nNonce   = 385454548;
        }
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xba, 0xd2 }; // Byt3coin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("Byt3coinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("Byt3coinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static Byt3coinMiner(CWallet *pwallet)
{
    printf("Byt3coinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("byt3coin-miner");
    ...
    try { loop {
        ...
        printf("Running Byt3coinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("Byt3coinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&Byt3coinMiner, pwallet));
}
